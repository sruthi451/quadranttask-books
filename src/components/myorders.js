...
class Myorders extends React.Component {
    ...
    render() {
        const date = new Date()
        var NamesofMonth = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "July", "Aug", "Sept", "Oct", "Nov", "Dec"];
        console.log(date)
        let mon = NamesofMonth[date.getMonth()]
        console.log("The current month  " + NamesofMonth[date.getMonth()]);
        console.log("The current year  " + date.getFullYear());
        const monthyear = mon + date.getFullYear();
        const year = date.getFullYear();
        let latestdate = date.getDate()
        const datemonthyear = latestdate + mon + year
        console.log("latestdate",datemonthyear)
        const myorders = this.props.orderdata.length > 0 ? (
            ...
            this.props.orderdata.map(item => {
                ...
                <h5>Place orders-list : {latestdate}{'-'}{mon}{'-'}{year}</h5>
                                            </div>
